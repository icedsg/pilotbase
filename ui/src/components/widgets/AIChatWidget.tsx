...
import { GripVertical, Send, Trash2, Bot } from 'lucide-react'
...
export default function AIChatWidget() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Response arrives via WebSocket → handled in useWebSocket
    } catch {
      addChatMessage({
        id: crypto.randomUUID(),
        role: 'assistant',
  ...
  return (
    <div ref={setNodeRef} style={{ ...style, minHeight: '240px' }} className="widget-card flex-1 min-h-0">
      <div className="widget-header">
        ...
        <button onClick={clearChat} className="btn-ghost p-0.5" title="Clear chat">
          <Trash2 size={12} />
        </button>
      </div>
      ...
      {/* Input */}
      <div className="p-2 border-t border-surface-50">
